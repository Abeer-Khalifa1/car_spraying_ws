...
class SprayQualityInspectorV2:
    ...
    def detect_binary_coverage(self, hsv, roi_mask):
        ...
        # Threshold paint colour using the configured (or previously
    ...
    def inspect(self, img, roi_mask=None):
        ...
        save_dir = "/home/user/car_spraying_ws/src/ob_detection/ob_detection/spray_paths"
...
def main(argv=None):
    ...
    try:
        ...
        max_frame_timeout = 100  # ~5 seconds at 0.05s sleep
        while True:
            if args.ros:
                ...
                if frame is None:
                    ...
                    if frame_timeout_count >= max_frame_timeout:
                        print(f'WARNING: No camera frames for {max_frame_timeout * 0.05:.1f}s. '
                              f'Check if {args.camera_topic} is being published.')
                        frame_timeout_count = 0
                    time.sleep(0.05)
                    continue
                frame_timeout_count = 0
                ...
                if not ret or frame is None:
                    ...
                    if frame_timeout_count % 50 == 0:
                        print(f'Failed to grab frame from camera (attempt {frame_timeout_count})...')
                    time.sleep(0.1)
                    continue
                frame_timeout_count = 0
            ...
            try:
                ...
                # --- gate on pass1_done: skip inspection until pass 1 finishes ---
                if args.ros and ros_reader is not None and not ros_reader.is_pass1_done():
                    cv2.putText(cam_display, 'Waiting for PASS 1 to complete...',
                                (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                (0, 165, 255), 2, cv2.LINE_AA)
                    cv2.imshow('Spray Quality Inspection', cam_display)
